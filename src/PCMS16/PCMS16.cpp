...
bool PCMS16FromADPCM_ADPCMXQ(const ADPCM_Header &header, const std::span<const char> &in, std::vector<int16_t> &out, int flags)
{
  ...
  if (!(flags & AudioConversionFlag::RAWOutput))
...
bool PCMS16ToADPCM_ADPCMXQ(const PCMS16_Header &header, const std::span<const int16_t> &in, std::vector<char> &out, int flags, int blocksizePow2, int lookahead)
{
  ...
  const auto result = ADPCMEncodeData(in, out, header.fmtChannels, num_samples, samples_per_block, lookahead,
      !(flags & AudioConversionFlag::ADPCMDisableNoiseShaping) ? (header.fmtSampleRate > 64000 ? NOISE_SHAPING_STATIC : NOISE_SHAPING_DYNAMIC) : NOISE_SHAPING_OFF);
...
{
  if (record.format == AudioDataFormat::PCM_S16 && record.bitsPerSample == 16)
  {
...
AudioDataInfo PCMS16SoundRecord(const PCMS16_Header &header, const uint64_t xxh3Hash)
{
  return {
    xxh3Hash,
    header.dataSize,
    ...
    static_cast<AudioDataFormat>(header.fmtFormat),
...
ADPCM_Header ADPCMHeader(const AudioDataInfo &in, int blocksizePow2)
{
  const auto samplesCount = static_cast<uint32_t>(in.dataSizeUncompressed / (in.channels * sizeof(int16_t)));

  if (in.format == AudioDataFormat::IMA_ADPCM && in.bitsPerSample == 4)
  {
    ADPCM_Header header;
...
AudioDataInfo ADPCMSoundRecord(const ADPCM_Header &header, const uint64_t xxh3Hash)
{
  return {
    xxh3Hash,
...
AudioDataInfo ADPCMSoundRecord(const ADPCM_Header &header, const std::span<const char> &in, bool pcms16XXH3Hash)
{
  if (!header.dataSize || in.size() < header.dataSize)
    return {};
  ...
  if (!PCMS16FromADPCM(header, in, pcms16Decoded, AudioConversionFlag::RAWOutput))
    return {};
...
AudioDataInfo VorbisSoundRecord(const AudioDataInfo &header, const uint64_t xxh3Hash)
{
  return {
    xxh3Hash,
...
AudioDataInfo VorbisSoundRecord(const AudioDataInfo &header, const std::span<const char> &in, bool pcms16XXH3Hash)
{
  if (!header.dataSize || in.size() < header.dataSize)
    return {};
  ...
  std::vector<int16_t> pcms16Decoded;
  if (!PCMS16FromVorbis(header, in, pcms16Decoded, AudioConversionFlag::RAWOutput))
    return {};
...
AudioDataInfo UnknownSoundDataHeader(const std::span<const char> &in)
{
  ...
  return {
    ...
    static_cast<uint32_t>(in.size()),
    sampleRate,
    AudioDataFormat::UNKNOWN_SUPPORTED,
    16,
    channels,
    static_cast<uint16_t>(channels * sizeof(int16_t)),
    channels
...
AudioDataInfo UnknownSoundDataSoundRecord(const AudioDataInfo &header, const uint64_t xxh3Hash)
{
  return {
    xxh3Hash,
    header.dataSizeUncompressed,
...
AudioDataInfo UnknownSoundDataSoundRecord(const AudioDataInfo &header, const std::span<const char> &in, bool pcms16XXH3Hash)
{
  if (!header.dataSize || in.size() < header.dataSize)
    return {};
  ...
  std::vector<int16_t> pcms16Decoded;
  if (!PCMS16FromUnknownSoundData(header, in, pcms16Decoded, AudioConversionFlag::RAWOutput))
    return {};
...
AudioDataInfo SoundDataSoundRecord(const AudioDataInfo &header, const uint64_t xxh3Hash)
{
  return {
    xxh3Hash,
    header.dataSizeUncompressed,
...
AudioDataInfo SoundDataSoundRecord(const AudioDataInfo &header, const std::span<const char> &in, bool pcms16XXH3Hash)
{
  if (!header.dataSize || in.size() < header.dataSize)
    return {};

  if (!pcms16XXH3Hash || header.format == AudioDataFormat::PCM_S16)
  {
    const auto dataXXH3Hash = XXH3_64bits(in.data(), header.dataSize);
  ...
  std::vector<int16_t> pcms16Decoded;
  if (!PCMS16FromSoundData(header, in, pcms16Decoded, AudioConversionFlag::RAWOutput))
    return {};
...
bool PCMS16FromVorbis(const AudioDataInfo &header, const std::span<const char> &in, std::vector<int16_t> &out, int flags)
{
  if (!header.dataSize || in.size() < header.dataSize)
    return false;
...
bool PCMS16FromUnknownSoundData(const AudioDataInfo &header, const std::span<const char> &in, std::vector<int16_t> &out, int flags)
{
  if (!header.dataSize || in.size() < header.dataSize)
    return false;
...
bool PCMS16FromSoundData(const AudioDataInfo &header, const std::span<const char> &in, std::vector<int16_t> &out, int flags)
{
  if (!header.dataSize || in.size() < header.dataSize)
    return false;
